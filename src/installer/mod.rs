...
use crate::{
  ...
  installer::{keyboard::{resolve, KEYMAPS}, users::User},
  split_hor, split_vert, styled_block, ui_back, ui_close, ui_down, ui_enter, ui_left, ui_right,
  ui_up,
  widget::{
    Button, ConfigWidget, HelpModal, InfoBox, InstallSteps, LineEditor, LogBox,
...
impl KeyboardLayout {
  pub fn new() -> Self {
    ...
    let mut layouts = StrList::new("Select Keyboard Layout", items);
    layouts.focus();
...
impl Page for KeyboardLayout {
  ...
  fn handle_input(&mut self, installer: &mut Installer, event: KeyEvent) -> Signal {
    match event.code {
      ...
      ui_back!() => Signal::Pop,
      KeyCode::Enter => {
        installer.keyboard_layout = KEYMAPS
            .get(self.layouts.selected_idx)
...
impl<'a> InstallProgress<'a> {
  pub fn new(
    ...
    steps.log_path = Some(std::path::PathBuf::from(log_path.clone()));
    ...
    let help_content = styled_block(vec![
      ...
      vec![(
        None,
        "This page shows the progress of the Athena OS installation process.",
      )],
      vec![(
        None,
  ...
  fn install_commands(
    ...
    Ok(vec![
			(Line::from("Beginning Athena OS Installation..."),
			vec![
...
impl<'a> Page for InstallProgress<'a> {
  fn render(&mut self, _installer: &mut Installer, f: &mut Frame, area: Rect) {
    ...
    if self.steps.is_complete() && !self.steps.has_error() {
